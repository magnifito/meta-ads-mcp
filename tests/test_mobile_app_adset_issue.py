...
class MobileAppAdsetTester:
    ...
    def test_reproduce_mobile_app_error(self) -> dict[str, Any]:
        ...
        if not result["success"]:
            return {"success": False, "error": f"MCP call failed: {result.get('text', 'Unknown error')}"}
        ...
        try:
            ...
            if "error" in parsed_content:
                ...
                if isinstance(error_details, dict) and "details" in error_details:
                    ...
                    if isinstance(meta_error, dict) and "error" in meta_error:
                        ...
                        is_dataset_error = error_code == 100 and "conversion event" in error_message.lower()
            ...
            return {"success": True, "reproduced_error": False, "unexpected_response": parsed_content}

        except json.JSONDecodeError as e:
            return {"success": False, "error": f"Failed to parse response: {e}", "raw_content": content}
