...
class AudienceEstimationTester:
    ...
    def __init__(self, base_url: str = "http://localhost:8080"):
        ...
        self.test_targeting_specs = {
            "simple_demographics": {"age_min": 25, "age_max": 65, "geo_locations": {"countries": ["US"]}},
    ...
    def test_pl_only_reachestimate_bounds(self) -> dict[str, Any]:
        ...
        if not parsed_content.get("success", False):
            print("   ❌ Response indicates failure but no error message found")
        ...
        ok = (
            ...
            and upper == expected_upper
            and midpoint == expected_midpoint
            and (fallback_used is None)
        )

        if ok:
            print(f"   ✅ Bounds: {lower:,}–{upper:,}; midpoint: {midpoint:,}")
            return {"success": True, "users_lower_bound": lower, "users_upper_bound": upper, "midpoint": midpoint}
        else:
            print(
                f"   ❌ Unexpected values: lower={lower}, upper={upper}, midpoint={midpoint}, fallback={fallback_used}"
            )
            return {
                "success": False,
    ...
    def test_comprehensive_audience_estimation(self) -> dict[str, Any]:
        ...
        for spec_name, targeting_spec in self.test_targeting_specs.items():
            ...
            if not result["success"]:
                results[spec_name] = {"success": False, "error": result.get("text", "Unknown error")}
            ...
            try:
                ...
                results[spec_name] = {"success": False, "error": "Invalid JSON response", "raw_content": content}
    ...
    def test_backwards_compatibility_interest_validation(self) -> dict[str, Any]:
        ...
        print("   📝 Testing interest name validation")

        result = self._make_request(
            "tools/call",
            {"name": "estimate_audience_size", "arguments": {"interest_list": self.test_interests["mixed_validity"]}},
        )
        ...
        print("   🔢 Testing interest FBID validation")

        result = self._make_request(
            "tools/call",
            {"name": "estimate_audience_size", "arguments": {"interest_fbid_list": self.test_interests["valid_fbids"]}},
        )
    ...
    def test_error_handling(self) -> dict[str, Any]:
        ...
        result = self._make_request("tools/call", {"name": "estimate_audience_size", "arguments": {}})
        ...
        print("   🚫 Testing account ID without targeting")
        result = self._make_request(
            "tools/call", {"name": "estimate_audience_size", "arguments": {"account_id": self.account_id}}
        )
        ...
        print("   🚫 Testing invalid targeting structure")
        result = self._make_request(
            "tools/call",
            {
                "name": "estimate_audience_size",
                "arguments": {"account_id": self.account_id, "targeting": {"invalid": "structure"}},
            },
        )
        ...
        print("   🚫 Testing missing location in targeting")
        result = self._make_request(
            "tools/call",
            {
                "name": "estimate_audience_size",
    ...
    def run_audience_estimation_tests(self) -> bool:
        ...
        comprehensive_success = any(
            result.get("success") and result.get("estimated_size", 0) > 0 for result in comprehensive_results.values()
        )
        ...
        compat_success = compat_results.get("interest_names", {}).get("success", False) and compat_results.get(
            "interest_fbids", {}
        ).get("success", False)
        ...
        goals_success = any(
            result.get("success") and result.get("estimated_size", 0) > 0 for result in goals_results.values()
        )
        ...
        error_success = all(result.get("success", False) for result in error_results.values())
