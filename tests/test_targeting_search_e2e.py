...
class TargetingSearchTester:
    ...
    def test_search_interests(self) -> dict[str, Any]:
        ...
        for query in self.test_queries["interests"]:
            ...
            result = self._make_request(
                "tools/call", {"name": "search_interests", "arguments": {"query": query, "limit": 5}}
            )
            ...
            try:
                ...
                if "error" in parsed_content:
                    results[query] = {"success": False, "error": parsed_content["error"]}
                ...
                results[query] = {
                    ...
                    "has_required_fields": all("id" in interest and "name" in interest for interest in interests),
                }
                ...
                results[query] = {"success": False, "error": "Invalid JSON response", "raw_content": content}
    ...
    def test_get_interest_suggestions(self) -> dict[str, Any]:
        ...
        result = self._make_request(
            "tools/call",
            {"name": "get_interest_suggestions", "arguments": {"interest_list": interest_list, "limit": 5}},
        )
        ...
        try:
            ...
            result_data = {
                ...
                "has_required_fields": all("id" in suggestion and "name" in suggestion for suggestion in suggestions),
            }
    ...
    def test_validate_interests(self) -> dict[str, Any]:
        ...
        result = self._make_request(
            "tools/call", {"name": "validate_interests", "arguments": {"interest_list": test_interests}}
        )
        ...
        try:
            ...
            result_data = {
                ...
                "has_valid_interests": any(validation.get("valid", False) for validation in validations),
                "has_invalid_interests": any(not validation.get("valid", True) for validation in validations),
            }
    ...
    def test_search_behaviors(self) -> dict[str, Any]:
        ...
        result = self._make_request("tools/call", {"name": "search_behaviors", "arguments": {"limit": 5}})
        ...
        try:
            ...
            result_data = {
                ...
                "has_required_fields": all("id" in behavior and "name" in behavior for behavior in behaviors),
            }
    ...
    def test_search_demographics(self) -> dict[str, Any]:
        ...
        for demo_class in self.test_queries["demographics"]:
            ...
            result = self._make_request(
                "tools/call",
                {"name": "search_demographics", "arguments": {"demographic_class": demo_class, "limit": 3}},
            )

            if not result["success"]:
                results[demo_class] = {"success": False, "error": result.get("text", "Unknown error")}
                print(f"   ❌ Failed: {result.get('text', 'Unknown error')}")
                continue
            ...
            try:
                ...
                if "error" in parsed_content:
                    results[demo_class] = {"success": False, "error": parsed_content["error"]}
                    print(f"   ❌ API Error: {parsed_content['error']}")
                    continue
                ...
                results[demo_class] = {
                    ...
                    "has_required_fields": all("id" in demo and "name" in demo for demo in demographics),
                }
                ...
                results[demo_class] = {"success": False, "error": "Invalid JSON response", "raw_content": content}
                print(f"   ❌ Invalid JSON: {content}")
        ...
        for query in self.test_queries["geo_locations"]:
            ...
            result = self._make_request(
                "tools/call",
                {
                    ...
                    "arguments": {"query": query, "location_types": ["country", "region", "city"], "limit": 3},
                },
            )
            ...
            try:
                ...
                results[query] = {
                    ...
                    "has_required_fields": all(
                        "key" in location and "name" in location and "type" in location for location in locations
                    ),
                }
                ...
                for location in locations[:3]:
                    print(
                        f"      • {location.get('name', 'N/A')} ({location.get('type', 'N/A')}, Key: {location.get('key', 'N/A')})"
                    )
    ...
    def run_targeting_search_tests(self) -> bool:
        ...
        interests_success = any(
            result.get("success") and result.get("count", 0) > 0 for result in interests_results.values()
        )
        ...
        validation_success = (
            ...
            and validation_result.get("has_valid_interests")
            and validation_result.get("has_invalid_interests")
        )
        ...
        demographics_success = any(
            result.get("success") and result.get("count", 0) > 0 for result in demographics_results.values()
        )
        ...
        geo_success = any(result.get("success") and result.get("count", 0) > 0 for result in geo_results.values())
