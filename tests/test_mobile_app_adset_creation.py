...
class TestMobileAppAdsetCreation:
    ...
    def mock_auth_manager(self):
        ...
        with (
            patch("meta_ads_mcp.core.api.auth_manager") as mock,
    ...
    async def test_mobile_app_adset_creation_success_ios(
        ...
        promoted_obj_param = (
            json.loads(params["promoted_object"])
            if isinstance(params["promoted_object"], str)
            else params["promoted_object"]
        )
    ...
    async def test_mobile_app_adset_with_pixel_tracking(
        ...
        await create_adset(
            **valid_mobile_app_params, promoted_object=promoted_object_with_pixel, destination_type="APP_STORE"
        )
    ...
    async def test_invalid_promoted_object_missing_store_url(
        ...
        invalid_promoted_object = {"application_id": "123456789012345", "custom_event_type": "APP_INSTALL"}
    ...
    async def test_invalid_destination_type_passes_through_to_meta(
        ...
        await create_adset(
            **valid_mobile_app_params, promoted_object=ios_promoted_object, destination_type="INVALID_TYPE"
        )
    ...
    @pytest.mark.asyncio
    async def test_ios_app_store_url_validation(self, mock_api_request, mock_auth_manager, valid_mobile_app_params):
    ...
    @pytest.mark.asyncio
    async def test_google_play_url_validation(self, mock_api_request, mock_auth_manager, valid_mobile_app_params):
    ...
    @pytest.mark.asyncio
    async def test_invalid_store_url_format(self, mock_api_request, mock_auth_manager, valid_mobile_app_params):
    ...
    async def test_deeplink_destination_type(
        ...
        await create_adset(**valid_mobile_app_params, promoted_object=ios_promoted_object, destination_type="DEEPLINK")
    ...
    async def test_app_install_destination_type(
        ...
        await create_adset(
            **valid_mobile_app_params, promoted_object=ios_promoted_object, destination_type="APP_INSTALL"
        )
    ...
    async def test_on_ad_destination_type_for_lead_generation(
        ...
        lead_gen_params.update({"optimization_goal": "LEAD_GENERATION", "billing_event": "IMPRESSIONS"})

        await create_adset(**lead_gen_params, destination_type="ON_AD")
    ...
    @pytest.mark.asyncio
    async def test_on_ad_validation_passes(self, mock_api_request, mock_auth_manager, valid_mobile_app_params):
        ...
        result = await create_adset(**lead_gen_params, destination_type="ON_AD")
    ...
    # Test: Error Handling
    @pytest.mark.asyncio
    async def test_meta_api_error_handling(self, mock_auth_manager, valid_mobile_app_params, ios_promoted_object):
        ...
        with patch("meta_ads_mcp.core.adsets.make_api_request") as mock_api:
            ...
            if "data" in result_data:
                ...
                assert (
                    "dataset" in error_text
                    or "conversion event" in error_text
                    or "dataset" in details_text
                    or "conversion event" in details_text
                )
            else:
                assert "error" in result_data
    ...
    @pytest.mark.asyncio
    async def test_backward_compatibility_non_mobile_campaigns(self, mock_api_request, mock_auth_manager):
        ...
        await create_adset(**non_mobile_params)
