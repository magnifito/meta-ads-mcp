...
class TestEstimateAudienceSize:
    ...
    async def test_comprehensive_audience_estimation_success(self):
            ...
            "data": [
                {
                    ...
                    "bid_estimates": {"median": 150, "min": 50, "max": 300},
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            mock_api.assert_called_once_with(
                "act_123456789/reachestimate", "test_token", {"targeting_spec": targeting_spec}, method="GET"
            )
    ...
    async def test_different_optimization_goals(self):
        ...
        mock_response = {
            "data": [{"estimate_mau": 800000, "estimate_dau": [], "bid_estimates": {}, "unsupported_targeting": []}]
        }

        targeting_spec = {"age_min": 18, "age_max": 45, "geo_locations": {"countries": ["US"]}}
    ...
    async def test_backwards_compatibility_interest_names(self):
        ...
        mock_response = {
            "data": [
                ...
                {"name": "invalidinterest", "valid": False},
            ]
        }
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await estimate_audience_size(access_token="test_token", interest_list=["Japan", "invalidinterest"])

            # Verify it uses the old validation API
            mock_api.assert_called_once_with(
                "search", "test_token", {"type": "adinterestvalid", "interest_list": '["Japan", "invalidinterest"]'}
            )
    ...
    async def test_backwards_compatibility_interest_fbids(self):
        ...
        mock_response = {"data": [{"id": "6003700426513", "valid": True, "audience_size": 68310258}]}

        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            mock_api.return_value = mock_response

            result = await estimate_audience_size(access_token="test_token", interest_fbid_list=["6003700426513"])

            # Verify it uses the old validation API
            mock_api.assert_called_once_with(
                "search", "test_token", {"type": "adinterestvalid", "interest_fbid_list": '["6003700426513"]'}
            )
    ...
    async def test_backwards_compatibility_both_interest_params(self):
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await estimate_audience_size(
                access_token="test_token", interest_list=["Japan"], interest_fbid_list=["6003397425735"]
            )
            ...
            mock_api.assert_called_once_with(
                "search",
                "test_token",
                {"type": "adinterestvalid", "interest_list": '["Japan"]', "interest_fbid_list": '["6003397425735"]'},
            )
    ...
    async def test_error_no_account_id_for_comprehensive(self):
        ...
        targeting_spec = {"age_min": 25, "age_max": 65, "geo_locations": {"countries": ["US"]}}

        result = await estimate_audience_size(access_token="test_token", targeting=targeting_spec)
    ...
    async def test_error_no_targeting_for_comprehensive(self):
        ...
        result = await estimate_audience_size(access_token="test_token", account_id="act_123456789")
    ...
    async def test_api_error_handling(self):
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await estimate_audience_size(
                access_token="test_token", account_id="act_123456789", targeting=targeting_spec
            )
    ...
    async def test_comprehensive_estimation_with_minimal_targeting(self):
        ...
        mock_response = {
            "data": [{"estimate_mau": 500000, "estimate_dau": [], "bid_estimates": {}, "unsupported_targeting": []}]
        }

        minimal_targeting = {"age_min": 18, "age_max": 65, "geo_locations": {"countries": ["US"]}}

        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            mock_api.return_value = mock_response

            result = await estimate_audience_size(
                access_token="test_token", account_id="act_123456789", targeting=minimal_targeting
            )
    ...
    async def test_estimate_details_structure(self):
            ...
            "data": [
                {
                    ...
                    "estimate_dau": [{"min_reach": 150000, "max_reach": 200000, "bid": 120}],
                    "bid_estimates": {"median": 180, "min": 80, "max": 350},
        ...
        targeting_spec = {
            ...
            "age_max": 45,
            "geo_locations": {"countries": ["US"]},
            "flexible_spec": [{"interests": [{"id": "6003371567474"}, {"id": "6003462346642"}]}],
        }
