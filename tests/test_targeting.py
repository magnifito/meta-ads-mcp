    ...
    async def test_search_interests_success(self):
        ...
        mock_response = {
            "data": [
                ...
                {"id": "6003397425735", "name": "Tennis", "audience_size": 987654321, "path": ["Sports", "Tennis"]},
            ]
        }
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            # Verify API call
            mock_api.assert_called_once_with("search", "test_token", {"type": "adinterest", "q": "movies", "limit": 10})
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            with patch("meta_ads_mcp.core.auth.get_current_access_token") as mock_auth:
                ...
                mock_api.assert_called_once_with(
                    "search", "test_token", {"type": "adinterest", "q": "test", "limit": 25}
                )
    ...
    async def test_get_interest_suggestions_success(self):
        ...
        mock_response = {
            "data": [
                ...
                {"id": "6003146664949", "name": "Netball", "audience_size": 4333770, "path": [], "description": None},
            ]
        }
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await get_interest_suggestions(
                access_token="test_token", interest_list=["Basketball", "Soccer"], limit=15
            )
            ...
            mock_api.assert_called_once_with(
                "search",
                "test_token",
                {"type": "adinterestsuggestion", "interest_list": '["Basketball", "Soccer"]', "limit": 15},
            )
    ...
    async def test_validate_interests_by_name_success(self):
        ...
        mock_response = {
            "data": [
                ...
                {"name": "nonexistantkeyword", "valid": False},
            ]
        }
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await estimate_audience_size(
                access_token="test_token", interest_list=["Japan", "nonexistantkeyword"]
            )

            # Verify API call
            mock_api.assert_called_once_with(
                "search", "test_token", {"type": "adinterestvalid", "interest_list": '["Japan", "nonexistantkeyword"]'}
            )
...
class TestSearchBehaviors:
    ...
    async def test_search_behaviors_success(self):
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            # Verify API call
            mock_api.assert_called_once_with(
                "search", "test_token", {"type": "adTargetingCategory", "class": "behaviors", "limit": 25}
            )
...
class TestSearchDemographics:
    ...
    async def test_search_demographics_success(self):
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await search_demographics(access_token="test_token", demographic_class="life_events", limit=30)

            # Verify API call
            mock_api.assert_called_once_with(
                "search", "test_token", {"type": "adTargetingCategory", "class": "life_events", "limit": 30}
            )
    ...
    async def test_search_geo_locations_success(self):
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            ...
            result = await search_geo_locations(
                access_token="test_token", query="United States", location_types=["country", "region"], limit=10
            )
            ...
            mock_api.assert_called_once_with(
                "search",
                "test_token",
                {"type": "adgeolocation", "q": "United States", "location_types": '["country", "region"]', "limit": 10},
            )
    ...
    async def test_search_geo_locations_no_location_types(self):
        ...
        with patch("meta_ads_mcp.core.targeting.make_api_request", new_callable=AsyncMock) as mock_api:
            with patch("meta_ads_mcp.core.auth.get_current_access_token") as mock_auth:
                ...
                mock_api.assert_called_once_with(
                    "search", "test_token", {"type": "adgeolocation", "q": "test", "limit": 25}
                )
