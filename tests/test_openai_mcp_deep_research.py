...
class OpenAIMCPTester:
    ...
    def test_search_tool_exists(self, auth_headers: dict[str, str] = None) -> dict[str, Any]:
        ...
        return {"success": search_tool is not None, "tool": search_tool, "all_tools": [tool["name"] for tool in tools]}
    ...
    def test_fetch_tool_exists(self, auth_headers: dict[str, str] = None) -> dict[str, Any]:
        ...
        return {"success": fetch_tool is not None, "tool": fetch_tool, "all_tools": [tool["name"] for tool in tools]}
    ...
    def test_search_tool_call(self, query: str, auth_headers: dict[str, str] = None) -> dict[str, Any]:
        ...
        result = self._make_request("tools/call", {"name": "search", "arguments": {"query": query}}, auth_headers)
        ...
        try:
            ...
            return {"success": True, "ids": ids, "raw_content": content, "id_count": len(ids)}
        except json.JSONDecodeError:
            return {"success": False, "error": "Search tool did not return valid JSON", "raw_content": content}
    ...
    def test_fetch_tool_call(self, record_id: str, auth_headers: dict[str, str] = None) -> dict[str, Any]:
        ...
        result = self._make_request("tools/call", {"name": "fetch", "arguments": {"id": record_id}}, auth_headers)
    ...
    def test_search_fetch_workflow(self, auth_headers: dict[str, str] = None) -> dict[str, Any]:
        ...
        if not search_result["success"]:
            return {"success": False, "step": "search", "error": search_result.get("error", "Search failed")}

        if not search_result["ids"]:
            return {"success": False, "step": "search", "error": "Search returned no IDs"}
    ...
    def run_openai_compliance_test_suite(self) -> bool:
        ...
        auth_scenarios = [{"name": "No Authentication", "headers": None}]
