...
@mcp_server.tool()
@meta_api_tool
async def search_interests(query: str, access_token: str | None = None, limit: int = 25) -> str:
    """
...
@mcp_server.tool()
@meta_api_tool
async def get_interest_suggestions(interest_list: list[str], access_token: str | None = None, limit: int = 25) -> str:
    """
        ...
        return json.dumps(
            {
                ...
                "example": {"geo_locations": {"countries": ["US"]}, "age_min": 25, "age_max": 65},
            },
            indent=2,
        )
    ...
    try:
        ...
        if isinstance(data, dict) and "error" in data:
            ...
            try:
                ...
                if isinstance(raw_err, dict) and (
                    raw_err.get("error_subcode") == 1885364
                    or raw_err.get("error_user_title") == "Missing Target Audience Location"
                ):
                    return json.dumps(
                        {
                            "error": "Missing target audience location",
                            "details": raw_err.get("error_user_msg")
                            ...
                            "blame_field_specs": raw_err.get("error_data", {}).get("blame_field_specs")
                            if isinstance(raw_err.get("error_data"), dict)
                            else None,
                        },
                        indent=2,
                    )
        ...
        if "100" in error_str and "33" in error_str:
            ...
            if targeting and "interests" in targeting:
                interests_found.extend(
                    [interest.get("id") for interest in targeting["interests"] if interest.get("id")]
                )
            elif targeting and "flexible_spec" in targeting:
                for spec in targeting["flexible_spec"]:
                    if "interests" in spec:
                        interests_found.extend(
                            [interest.get("id") for interest in spec["interests"] if interest.get("id")]
                        )
            ...
            if interests_found:
                ...
                try:
                    fallback_result = await estimate_audience_size(
                        access_token=access_token, interest_fbid_list=interests_found
                    )
...
@mcp_server.tool()
@meta_api_tool
async def search_behaviors(access_token: str | None = None, limit: int = 50) -> str:
    """
...
async def search_demographics(
    access_token: str | None = None, demographic_class: str = "demographics", limit: int = 50
) -> str:
    """
...
async def search_geo_locations(
    query: str, access_token: str | None = None, location_types: list[str] | None = None, limit: int = 25
) -> str:
    """
