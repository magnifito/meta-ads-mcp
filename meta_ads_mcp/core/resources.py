...
async def list_resources() -> dict[str, Any]:
    ...
    for resource_id, image_info in ad_creative_images.items():
        resources.append(
            {"uri": f"meta-ads://images/{resource_id}", "mimeType": image_info["mime_type"], "name": image_info["name"]}
        )
...
async def get_resource(resource_id: str) -> dict[str, Any]:
    ...
    if resource_id in ad_creative_images:
        ...
        return {"data": base64.b64encode(image_info["data"]).decode("utf-8"), "mimeType": image_info["mime_type"]}
