...
class TokenInfo:
    ...
    def __init__(self, access_token: str, expires_in: int | None = None, user_id: str | None = None):
    ...
    def deserialize(cls, data: dict[str, Any]) -> "TokenInfo":
        ...
        token = cls(
            access_token=data.get("access_token", ""), expires_in=data.get("expires_in"), user_id=data.get("user_id")
        )
...
class AuthManager:
    ...
    def _load_cached_token(self) -> bool:
        ...
        try:
            with open(cache_path) as f:
                ...
                logger.info(
                    f"Loaded cached token (expires in {(self.token_info.created_at + self.token_info.expires_in) - int(time.time())} seconds)"
                )
                return True
        except Exception as e:
    ...
    def authenticate(self, force_refresh: bool = False) -> str | None:
        """
...
def process_token_response(token_container):
    ...
    if token_container and token_container.get("token"):
        ...
        if long_lived_token_info:
            logger.info(
                f"Successfully exchanged for long-lived token (expires in {long_lived_token_info.expires_in} seconds)"
            )

            try:
                auth_manager.token_info = long_lived_token_info
                logger.info(
                    f"Long-lived token info set in auth_manager, expires in {long_lived_token_info.expires_in} seconds"
                )
            except NameError:
            ...
            token_info = TokenInfo(access_token=short_lived_token, expires_in=token_container.get("expires_in", 0))
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if new_token:
                ...
                return TokenInfo(access_token=new_token, expires_in=expires_in)
            else:
...
async def get_current_access_token() -> str | None:
    ...
    if env_token:
        ...
        if len(env_token) < 20:  # Most Meta tokens are much longer
            logger.error(
                f"TOKEN VALIDATION FAILED: Token from environment variable appears malformed (length: {len(env_token)})"
            )
            return None
        return env_token
