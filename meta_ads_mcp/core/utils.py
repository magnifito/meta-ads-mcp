...
if not os.environ.get("META_ACCESS_TOKEN"):
    ...
    if not META_APP_SECRET:
        print(
            "NOTE: META_APP_SECRET is not set. Required only to exchange for long-lived tokens via the local OAuth flow."
        )
...
async def try_multiple_download_methods(url: str) -> bytes | None:
    """
...
def create_resource_from_image(image_bytes: bytes, resource_id: str, name: str) -> dict[str, Any]:
    ...
    """
    ad_creative_images[resource_id] = {"data": image_bytes, "mime_type": "image/jpeg", "name": name}
