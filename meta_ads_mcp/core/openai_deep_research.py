...
class MetaAdsDataManager:
    ...
    async def _get_businesses(self, access_token: str, user_id: str = "me", limit: int = 25) -> list[dict[str, Any]]:
        ...
        try:
            ...
            params = {"fields": "id,name,created_time,verification_status", "limit": limit}
...
@mcp_server.tool()
@meta_api_tool
async def search(query: str, access_token: str | None = None) -> str:
    """
    ...
    try:
        ...
        response = {"ids": matching_ids, "query": query, "total_results": len(matching_ids)}
        ...
        return json.dumps(
            {"error": "Failed to search Meta Ads data", "details": error_msg, "ids": [], "query": query}, indent=2
        )
...
async def fetch(id: str) -> str:
    ...
    """
    if not id:
        return json.dumps({"error": "id parameter is required"}, indent=2)
    ...
    try:
        ...
        if record:
            ...
            return json.dumps({"error": f"Record not found: {id}", "id": id}, indent=2)

    except Exception as e:
        error_msg = str(e)
        logger.error(f"Error in fetch tool: {error_msg}")

        return json.dumps({"error": "Failed to fetch record", "details": error_msg, "id": id}, indent=2)
